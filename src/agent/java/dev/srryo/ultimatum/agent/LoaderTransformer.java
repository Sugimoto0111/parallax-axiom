...
public final class LoaderTransformer implements ClassFileTransformer {
    ...
    public byte[] transform(ClassLoader loader, String className, Class<?> redefining,
        ...
        try {
            ...
            for (MethodNode method : node.methods) {
                ...
                for (AbstractInsnNode instruction : method.instructions.toArray()) {
                    ...
                    hook.add(new MethodInsnNode(Opcodes.INVOKESTATIC,
                            "dev/srryo/ultimatum/agent/UniversalClassTransformer",
            ...
            node.accept(writer);
            System.out.println("[Ultimatum Agent] ModuleClassLoader hook installed");
            return writer.toByteArray();
        } catch (Throwable error) {
            installed.set(false);
            System.err.println("[Ultimatum Agent] Loader transformation failed: " + error);
            return null;
        }
    }
}

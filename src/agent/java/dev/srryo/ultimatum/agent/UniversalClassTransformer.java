package dev.srryo.ultimatum.agent;
 ...
 * three return values required by Ultimatum's forced-death state.
 */
public final class UniversalClassTransformer {
    private static final String HOOK =
            "dev/srryo/ultimatum/kill/hook/ExecutionMethodHooks";
    private static final String HOOK_INTERFACE =
            "dev/srryo/ultimatum/kill/hook/ExecutionMethodHook";
    ...
    public static byte[] transform(String requestedName, byte[] bytes) {
        ...
        try {
            ...
            if (node.name.startsWith("dev/srryo/ultimatum/agent/")
                    || node.name.startsWith("dev/srryo/ultimatum/kill/hook/")
            ...
            System.err.println("[Ultimatum Agent] Class transformation failed for "
                    + requestedName + ": " + error);
            return bytes;
        }
    }

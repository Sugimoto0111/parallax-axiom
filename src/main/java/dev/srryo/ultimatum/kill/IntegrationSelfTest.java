...
final class IntegrationSelfTest {
    private static final String MODE = System.getProperty("ultimatum.integrationTest", "");
    ...
    private static void startPig2(MinecraftServer server, KillService service) {
        ...
        targetLevel = level;
        UltimatumMod.LOGGER.info("[ULTIMATUM-IT] Spawned Pig2 {}; executing", targetUuid);
    ...
    private static void finish(MinecraftServer server, boolean success, String detail) {
        finished = true;
        if (success) {
            UltimatumMod.LOGGER.info("[ULTIMATUM-IT] PASS: {}", detail);
        } else {
            UltimatumMod.LOGGER.error("[ULTIMATUM-IT] FAIL: {}", detail);
        }
        server.halt(false);
    }
}

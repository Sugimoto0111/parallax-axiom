package dev.srryo.parallaxaxiom.agentloader;
...
public final class AgentLoader {
    ...
    public static synchronized boolean loadUnlessNoSugarAgentPresent() {
        ...
        try {
            ...
            System.out.println("[Parallax Axiom] NoSugar agent detected; sharing its method hooks");
            loaded = true;
            return true;
        ...
        try {
            ...
            if (agent == null) {
                System.err.println("[Parallax Axiom] Embedded agent resource was not found");
                return false;
            }
            agent.deleteOnExit();
            System.setProperty("parallax_axiom.agent.jar", agent.getAbsolutePath());
            ...
            loaded = true;
            System.out.println("[Parallax Axiom] Universal method-hook agent loaded");
            return true;
        } catch (Throwable error) {
            System.err.println("[Parallax Axiom] Could not load universal method-hook agent: "
                    + error);
            return false;
        }
    }
    ...
    private static File extractAgent() throws Exception {
        try (InputStream input = AgentLoader.class.getResourceAsStream(
                "/META-INF/jarjar/parallax-axiom-agent.jar")) {
            if (input == null) {
                return null;
            }
            File output = File.createTempFile("parallax-axiom-agent", ".jar");

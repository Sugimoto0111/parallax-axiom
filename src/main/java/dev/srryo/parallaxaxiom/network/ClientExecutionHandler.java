...
import dev.srryo.parallaxaxiom.kill.DeterministicWorldIndexEraser;
import dev.srryo.parallaxaxiom.kill.ExternalErasedStateBridge;
import dev.srryo.parallaxaxiom.kill.ForcedDeathState;
...
final class ClientExecutionHandler {
    ...
    private static void forceHealthZero(LivingEntity target) {
        EntityDataAccessor<Float> health = LivingEntityDeathAccessor.parallaxAxiom$getDataHealthId();
        SynchedEntityData data = target.getEntityData();
        try {
            SynchedEntityData.DataItem<Float> item =
                    ((SynchedEntityDataAccessor) data).parallaxAxiom$getItem(health);
            ((SynchedEntityDataItemAccessor<Float>) (Object) item).parallaxAxiom$setValue(0.0F);
